...
function createParser(data, pos) {
    const parser = new WmbusParser(IS_DEBUG ? undefined : () => {});
...
describe('Extended Link Layer', () => {
    ...
    describe('Compact frames', () => {
        it('Compact frame but no cached structure', () => {
            const parser = createParser(Buffer.from('79C4D788B0A60B00004E11000013070000C91A0000000000000000B10B6709', 'hex'), 0);

            expect(() => parser.decodeApplicationLayer()).to.throw('Found unknown compact frame header - waiting for full frame');
            expect(parser.unknownFrameCrcs).to.be.eql([0xD7C4]);
        });

        it('Full frame and cache needed', () => {
            const parser = createParser(Buffer.from('0406A60B000004FF074E11000004FF08130700000414C91A000002FD170000043B000000000259B10B025D6709', 'hex'), 0);
            parser.unknownFrameCrcs = [0xD7C4];
            parser.telegram.applicationLayer.cachedDataRecordHeaders = false;
            parser.decodeDataRecords();
            expect(parser.unknownFrameCrcs).to.be.empty;
            // @ts-ignore
            expect(parser.dataRecordHeaderCache).to.have.key(0xD7C4);
            expect(parser.dataRecordHeaderCache.get(0xD7C4)).to.have.lengthOf(8);
        });

        it('Compact frame with cache available', () => {
            const parser = createParser(Buffer.from('0406A60B000004FF074E11000004FF08130700000414C91A000002FD170000043B000000000259B10B025D6709', 'hex'), 0);
            parser.unknownFrameCrcs = [0xD7C4];
            parser.telegram.applicationLayer.cachedDataRecordHeaders = false;
            parser.decodeDataRecords();
            parser.resetTelegramData();

            parser.data = Buffer.from('79C4D788B0A60B00004E11000013070000C91A0000000000000000B10B6709', 'hex');
            parser.telegram.config.mode = 0;
            parser.decodeApplicationLayer();
            parser.decodeDataRecords();
            ...
            expect(result[7].stringValue).to.eql('');
            expect(result[7].type).to.eql('number');
            expect(result[7].value).to.eql(2407);

            expect(result[7].header).to.eql({
                dib: {
                    dataField: 2,
                    devUnit: 0,
                    functionField: 0,
