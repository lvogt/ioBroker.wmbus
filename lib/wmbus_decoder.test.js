...
describe('JMBus Test Cases', () => {
    beforeEach(() => {
        decoder = new WMBUS_DECODER(function() {}, false);
    });
    ...
    it('Short Telegram Test #1', () => {
        decoder = new WMBUS_DECODER(function() {}, true);
...
describe('OMS Examples', () => {
    beforeEach(() => {
        decoder = new WMBUS_DECODER(function() {});
    });
    ...
    it('Frame type B without CRC', () => {
        const msg = '1244AE0C7856341201078C2027780B13436587';
        decoder.parse(msg, false, undefined, 'B', (err, res) => {
            expect(res.deviceInformation.Manufacturer).to.be.equal('CEN');
            expect(res.deviceInformation.Medium).to.be.equal('Water');
            expect(res.deviceInformation.Id).to.be.equal('12345678');
            expect(res.dataRecord).to.have.lengthOf(1);
    ...
    it('DIF 64bit integer with calc', () => {
        const msg = '1744ae0c7856341201078c20277807138877665544332211';
        decoder.parse(msg, false, undefined, 'B', (err, res) => {
            expect(res.deviceInformation.Manufacturer).to.be.equal('CEN');
            expect(res.deviceInformation.Medium).to.be.equal('Water');
            expect(res.deviceInformation.Id).to.be.equal('12345678');
            expect(res.dataRecord).to.have.lengthOf(1);
            expect(Number(res.dataRecord[0].value)).to.be.closeTo(1234605616436508.552, 0.3);
        });
    });

    it('DIF 64bit integer as manufacturer specific', () => {
        const msg = '1744ae0c7856341201078c202778077F8877665544332211';
        decoder.parse(msg, false, undefined, 'B', (err, res) => {
            expect(res.deviceInformation.Manufacturer).to.be.equal('CEN');
            expect(res.deviceInformation.Medium).to.be.equal('Water');
            expect(res.deviceInformation.Id).to.be.equal('12345678');
            expect(res.dataRecord).to.have.lengthOf(1);
            expect(res.dataRecord[0].value).to.be.equal('1234605616436508552');
        });
    });
});
